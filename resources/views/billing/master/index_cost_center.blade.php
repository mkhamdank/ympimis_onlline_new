                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table user-table no-wrap" id="TableCostCenter">
                                <thead>
                                    <tr>
                                        <th style="background-color: #757ce8;">Cost Center</th>
                                        <th style="background-color: #757ce8;">Cost Center Name</th>
                                        <th style="background-color: #757ce8;">Code</th>
                                        <th style="background-color: #757ce8;">Remark</th>
                                        <th style="background-color: #757ce8;">Action</th>
                                    </tr>
                                </thead>
                               <tbody id="bodyTableCostCenter">
...
<script type="text/javascript">
    ...
    function fetchData() {
        $.get('{{ url("fetch/cost_center") }}',  function(result, status, xhr){
            if(result.status){
                $('#TableCostCenter').DataTable().clear();
                $('#TableCostCenter').DataTable().destroy();
                $("#bodyTableCostCenter").html('');
                var bodyTable = '';
                for (var i = 0; i < result.cost_center.length; i++) {
                    bodyTable += '<tr>';
                    bodyTable += '<td style="text-align:left">'+result.cost_center[i].cost_center+'</td>';
                    bodyTable += '<td style="text-align:left">'+result.cost_center[i].cost_center_name+'</td>';
                    bodyTable += '<td style="text-align:left">'+result.cost_center[i].code+'</td>';
                    bodyTable += '<td style="text-align:left">'+result.cost_center[i].remark+'</td>';
                    bodyTable += '<td></td>';
                    bodyTable += '</tr>';
                }
                $('#bodyTableCostCenter').append(bodyTable);

                var table = $('#TableCostCenter').DataTable({
                    'dom': 'Bfrtip',
                    'responsive':true,
                    'lengthMenu': [
                    [ 10, 25, 50, -1 ],
