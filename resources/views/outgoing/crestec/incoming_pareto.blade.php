...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/pareto/crestec") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					Highcharts.chart('container1', {
					    ...
					    title: {
					        text: 'PARETO INCOMING PT. CRESTEC INDONESIA',
					        style:{
					        	fontWeight:'bold'
					        }
					    },
	...
	function showModalDetailPareto(categories) {
		...
		$.get('{{ url("fetch/incoming/pareto/detail/crestec") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
