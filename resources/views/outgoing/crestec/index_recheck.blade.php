	...
	<div class="row" style="padding-left: 10px; padding-right: 5px;">
		<div class="col-md-6" style="padding-right: 10px; padding-left: 0">
			<table class="table table-bordered" style="width: 100%; margin-bottom: 2px;" border="1">
				<tbody>
					<tr>
						<th style=" background-color: #d1d1d1; text-align: center; color: #14213d; padding:0;font-size: 15px;width: 5%">Tanggal Recheck</th>
						<th style=" background-color: #d1d1d1; text-align: center; color: #14213d; padding:0;font-size: 15px;width: 5%">Tanggal Check</th>
					...
					<tr>
						...
						<td style="background-color: #fca311; color: #14213d; text-align: center; font-size:15px;"><input type="text" style="text-align: center;font-size: 18px" readonly name="check_date" id="check_date" class="form-control" value="{{$outgoing->check_date}}"></td>
						...
						<td style="background-color: none; color: #14213d; text-align: center; font-size:15px;">
							<input type="text" name="serial_number" class="form-control" id="serial_number" placeholder="Job Number" style="width: 100%;font-size: 18px;text-align: center;" value="{{$outgoing->serial_number}}">
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="col-md-6" style="padding-right: 0; padding-left: 5px">
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					...
					<tr>
						<td>
							<input type="number" class="pull-right numpad" name="qty_check" style="height: 30px;font-size: 1vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="qty_check" placeholder="Quantity Check"  value="{{$outgoing->qty_check}}">
						</td>
						<td>
							<input type="text" class="pull-right" name="total_ok" style="height: 30px;font-size: 1vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="total_ok" placeholder="Qty OK" readonly value="{{$outgoing->qty_check}}">
						</td>
					</tr>
				</tbody>
			</table>
...
<script>
	...
	jQuery(document).ready(function() {
		$('#material_number').val('{{$outgoing->material_number}} - {{$outgoing->material_description}}').trigger('change');
	...
	function confirmNgLog() {
		...
		if ($('#material_description').text() == "-" || $('#qty_check').val() == "" || $('#serial_number').val() == "") {
			...
			$.post('{{ url("index/outgoing/crestec/confirm/recheck") }}', data, function(result, status, xhr){
				if(result.status){
					cancelAll();
					// updateKensaCode();
					...
					window.location.replace('{{url("index/outgoing/crestec")}}');
				}else{
					openErrorGritter('Error!',result.message);
					$('#loading').hide();
					$('#btn_confirm').removeAttr('disabled');
