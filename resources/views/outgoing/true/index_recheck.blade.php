			...
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="number" class="pull-right numpad" name="qty_check" style="height: 50px;font-size: 2vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="qty_check" placeholder="Quantity Check" onchange="checkQty(this.value);" value="{{$outgoing[0]->qty_check}}">
						</td>
					</tr>
				</tbody>
			</table>
...
<script>
	...
	function confirmNgLog() {
		...
		if ($('#material_description').text() == "-" || $('#qty_check').val() == "") {
			...
			$.post('{{ url("index/outgoing/true/confirm/recheck") }}', data, function(result, status, xhr){
				if(result.status){
					cancelAll();
					// updateKensaCode();
