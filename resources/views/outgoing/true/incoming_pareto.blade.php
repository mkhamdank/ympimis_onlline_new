...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/pareto/true") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					Highcharts.chart('container1', {
					    ...
					    title: {
					        text: 'PARETO DEFECT PT. TRUE<br>'+result.firstMonthTitle+' - '+result.lastMonthTitle
					    },
					    tooltip: {
					        shared: true
					    },
	...
	function showModalDetailPareto(categories) {
		...
		$.get('{{ url("fetch/incoming/pareto/detail/true") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
