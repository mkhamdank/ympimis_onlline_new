...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/ng_rate/true") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
					...
					Highcharts.chart('container', {
					    ...
					    title: {
					        text: 'Incoming Check NG Rate PT. TRUE'
					    },
					    subtitle: {
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/incoming/ng_rate/detail/true") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
