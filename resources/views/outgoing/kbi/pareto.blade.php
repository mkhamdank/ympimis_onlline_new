...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/outgoing/pareto/kbi") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
	...
	function showModalDetailPareto(categories) {
		...
		$.get('{{ url("fetch/outgoing/pareto/detail/kbi") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
