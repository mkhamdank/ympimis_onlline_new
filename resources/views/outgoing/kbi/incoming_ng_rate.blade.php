...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/ng_rate/kbi") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
					...
					Highcharts.chart('container', {
					    ...
					    title: {
					        text: 'Incoming Check NG Rate PT. KBI'
					    },
					    subtitle: {
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/incoming/ng_rate/detail/kbi") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
