...
<script>
	...
	function updateKensaCode() {
		$.get('{{ url("fetch/kensa/serial_number/rk") }}',  function(result, status, xhr){
			if(result.status){
	...
	function selectMaterial(material) {
		...
		// $.get('{{ url("fetch/outgoing/rk/material") }}', data, function(result, status, xhr){
		// 	if(result.status){
	...
	function confirmNgLog() {
		...
		if ($('#material_description').text() == "-" || $('#qty_check').val() == "") {
			...
			$.post('{{ url("index/outgoing/rk/confirm") }}', data, function(result, status, xhr){
				if(result.status){
					cancelAll();
					updateKensaCode();
