...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/rk") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/detail/rk") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
