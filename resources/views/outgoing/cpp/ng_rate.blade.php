...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/cpp") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
					...
					Highcharts.chart('container', {
					    ...
					    title: {
					        text: 'Production NG Rate PT. CONTINENTAL PANJIPRATAMA'
					    },
					    subtitle: {
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/detail/cpp") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
