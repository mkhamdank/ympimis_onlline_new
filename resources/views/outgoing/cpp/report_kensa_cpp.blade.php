...
<script type="text/javascript">
    ...
    function fetchData() {
        ...
        $.get('{{ url("fetch/kensa/cpp/report") }}',data,  function(result, status, xhr){
            if(result.status){
                ...
                for (var i = 0; i < result.outgoing.length; i++) {
                    ...
                    bodyTable += '<td>'+(result.outgoing[i].lot_status || 'LOT OK')+'</td>';
                    if (result.outgoing[i].lot_status == 'LOT OUT' && result.outgoing[i].recheck_status == 'Checked') {
                        ...
                        var url = '{{url("index/outgoing/cpp/input/lot_out/")}}'+'/'+result.outgoing[i].serial_number+'/'+result.outgoing[i].check_date;
