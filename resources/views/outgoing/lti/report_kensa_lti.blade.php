...
<script type="text/javascript">
    ...
    function fetchData() {
        ...
        $.get('{{ url("fetch/kensa/lti/report") }}',data,  function(result, status, xhr){
            if(result.status){
                ...
                for (var i = 0; i < result.outgoing.length; i++) {
                    ...
                    if (result.outgoing[i].lot_status == 'LOT OUT' && result.outgoing[i].recheck_status == 'Checked') {
                        ...
                        var url = '{{url("index/outgoing/lti/input/lot_out/")}}'+'/'+result.outgoing[i].serial_number+'/'+result.outgoing[i].check_date;
