...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/outgoing/pareto/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					Highcharts.chart('container1', {
					    ...
					    title: {
					        text: 'PARETO DEFECT PT. LIMA TEKNO INDONESIA',
					        style:{
					        	fontWeight:'bold'
					        }
					    },
	...
	function showModalDetailPareto(categories) {
		...
		$.get('{{ url("fetch/outgoing/pareto/detail/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
