...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/pareto/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					Highcharts.chart('container1', {
					    ...
					    title: {
					        text: 'PARETO INCOMING PT. LIMA TEKNO INDONESIA',
					        style:{
					        	fontWeight:'bold'
					        }
					    },
	...
	function showModalDetailPareto(categories) {
		...
		$.get('{{ url("fetch/incoming/pareto/detail/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
