...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/incoming/ng_rate/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
					...
					Highcharts.chart('container', {
					    ...
					    title: {
					        text: 'Incoming Check NG Rate PT. LIMA TEKNO INDONESIA'
					    },
					    subtitle: {
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/incoming/ng_rate/detail/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
