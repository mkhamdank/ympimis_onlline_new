...
<script>
	...
	function fetchData() {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					var categories = [];
					var checkes = [];
					...
					Highcharts.chart('container', {
					    ...
					    title: {
					        text: 'Production NG Rate PT. LIMA TEKNO INDONESIA'
					    },
					    subtitle: {
	...
	function showModalDetail(categories) {
		...
		$.get('{{ url("fetch/outgoing/ng_rate/detail/lti") }}',data, function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					$.each(result.outgoing, function(key,value){
						...
						bodyDetail += '<td style="padding:2px;font-size:13px;border:1px solid black;text-align:left;padding-left:7px;">'+value.ng_name+'</td>';
